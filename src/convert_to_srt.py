...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fix malformed SRT timestamps.")
    parser.add_argument('-i', '--input', type=str, help='Path to input SRT file')
    parser.add_argument('-o', '--output', type=str, help='Path to output SRT file (default: output.srt)')
    args = parser.parse_args()

    input_file = args.input
    output_file = args.output or "output.srt"
    backup_file = None

    if not input_file:
        input_file = input("Enter path to input SRT file: ").strip()
    if not output_file:
        output_file = input("Enter path to output SRT file [output.srt]: ").strip() or "output.srt"
